...
const details = [
  ["32-trust-score-midnight.png", "Verify", "A score. And the reasons.", "Look beyond the number. Review the evidence and the context behind each finding.", "assistance"],
      ...
      <div className="home-macro-details-grid">
        {details.map(([file, label, title, description, anchor]) => (
          <a href={`/macrokii#${anchor}`} key={file}>
            <img src={`/assets/macrokii/details/${file}`} width="2752" height="2064" alt={`${label}: enlarged feature detail alongside the app`} loading="lazy" />
